...
type SelectTriggerProps = React.ComponentProps<typeof SelectPrimitive.Trigger> & {
  /** Altura do gatilho: `sm` (h-8) ou `default` (h-9). */
  size?: 'sm' | 'default'
}
...
function SelectTrigger({ className, size = 'default', children, ...props }: SelectTriggerProps) {
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        'flex w-full items-center justify-between gap-2 rounded-lg border border-input bg-background',
        'px-3 py-1 text-sm shadow-xs transition',
        size === 'sm' ? 'h-8' : 'h-9',
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        position={position}
        className={cn(
          'relative z-50 max-h-(--radix-select-content-available-height) min-w-32 overflow-x-hidden overflow-y-auto',
